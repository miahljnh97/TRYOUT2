...
namespace NotificationService.Application.NotificationMediator.Queries.GetWithLog
{
    ...
    {
        private NotifContext _context;
        public GetWithLogQueryHandler(NotifContext context)
        {
            _context = context;
        }
        ...
        {
            var notifData = await _context.notification.ToListAsync();
            var notifLogData = await _context.notificationLogs.ToListAsync();
